# 0918_v1及之前的版本适用，只是增加了一些数据类，没有本质的修改
import os
import torch
import random
import logging
...
class BaseDataset(Dataset):
    ...
    def unpack(self, to_be_unpacked):
        ...
        #xiugai 聚类
        # history_ts = torch.tensor([_[6] for _ in to_be_unpacked], device=self.device)
        # target_ts = torch.tensor([_[7] for _ in to_be_unpacked], device=self.device)
        return user_id, user_seq, target_item, seq_len, label, domain_id
        # return user_id, user_seq, target_item, seq_len, label, domain_id, history_ts,target_ts
        # if self.phase != 'train':
        #     user_hist = user_seq
...
class SeparateDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        batch = {}
        batch[self.fuid] = data[0][idx] #user_id
        batch['in_' + self.fiid] = data[1][idx] #history_items
        batch[self.fiid] = data[2][idx] #target_items
        batch['seqlen'] = data[3][idx] #seq_len
        batch['label'] = data[4][idx] #label
        batch['domain_id'] = data[5][idx] #domain_id
        batch['index'] = idx
        # if self.phase != 'train':
        #     batch['user_hist'] = data[6][idx]
        # batch['in_' + self.fiid + '_ts'] = data[6][idx] #history_ts
        # batch[self.fiid + '_ts'] = data[7][idx] #target_ts
        if self.phase != 'train':
...
#     def get_loader(self, batch_size=None, shuffle=True, filter_type=None):
#         """
#         创建一个数据加载器 DataLoader。
#         可以根据 filter_type 的值来筛选数据。

#         Args:
#             batch_size (int): 批次大小。
#             shuffle (bool): 是否打乱数据。
#             filter_type (str, optional): 筛选类型。
#                 - None: 加载所有数据。
#                 - 'simple': 只加载 user_id == 0 的数据。
#                 - 'other': 只加载 user_id != 0 的数据。
#                 默认为 None。

#         Returns:
#             DataLoader: PyTorch 数据加载器。
#         """
#         # <--- 修改1：如果调用时未提供batch_size，则从config中自动获取
#         if batch_size is None:
#             if self.phase == 'train':
#                 batch_size = self.config['train']['batch_size']
#             else: # 'validation' or 'test' phase
...
#         all_user_ids = self.data[0] 

#         if filter_type == 'simple':
#             indices = torch.where(all_user_ids == 0)[0]
...
#         self.logger.info(f"Created a DataLoader with filter '{filter_type}'. Total samples: {len(subset)}")
#         return DataLoader(subset, batch_size=batch_size, shuffle=shuffle)

class CurriculumDataset(BaseDataset):
    # In CurriculumDataset class
    # 确保文件顶部有: from torch.utils.data import DataLoader, Subset
    # 并且: import torch

    def __init__(self, config, phase='train'): # 在 __init__ 方法中增加两个属性
        super().__init__(config, phase)
        # 缓存索引，避免重复计算，提升效率
        self._proto_indices = None
        self._real_indices = None
    ...
    def get_loader(self, batch_size=None, shuffle=True, mix_ratio=None):
        """
        创建一个数据加载器 DataLoader。
        支持按 mix_ratio 混合原型数据和真实数据。
        """
        if batch_size is None:
            if self.phase == 'train':
                batch_size = self.config['train']['batch_size']
        ...
        # 对于非训练阶段或没有混合比例时，返回包含所有数据的加载器
        if self.phase != 'train' or mix_ratio is None:
            return DataLoader(self, batch_size=batch_size, shuffle=shuffle)

        # --- 核心：按比例混合采样 ---

        # 1. 首次调用时，计算并缓存两类数据的索引
        if self._proto_indices is None:
            self.logger.info("First time sampling, calculating and caching indices...")
            all_user_ids = self.data[0]
            self._proto_indices = torch.where(all_user_ids == 0)[0]
            self._real_indices = torch.where(all_user_ids != 0)[0]
            self.logger.info(f"Found {len(self._proto_indices)} prototype samples and {len(self._real_indices)} real samples.")

        proto_ratio, real_ratio = mix_ratio
        
        # 2. 根据比例计算需要采样的数量
        num_proto = int(len(self._proto_indices) * proto_ratio)
        num_real = int(len(self._real_indices) * real_ratio)

        # 3. 对两类索引分别进行随机排列，以确保每次 epoch 的数据都不同
        shuffled_proto_indices = self._proto_indices[torch.randperm(len(self._proto_indices))]
        shuffled_real_indices = self._real_indices[torch.randperm(len(self._real_indices))]

        # 4. 取出相应数量的索引
        sampled_proto = shuffled_proto_indices[:num_proto]
        sampled_real = shuffled_real_indices[:num_real]
        
        # 5. 合并索引
        combined_indices = torch.cat([sampled_proto, sampled_real])

        # 6. 使用合并后的索引创建 Subset
        subset = Subset(self, combined_indices.tolist())
        
        self.logger.info(f"Created a mixed DataLoader. Ratio: [Proto:{proto_ratio}, Real:{real_ratio}]. Samples: {len(subset)} (Proto:{num_proto}, Real:{num_real})")
        
        # 对混合后的数据进行 shuffle=True 非常重要，可以确保每个 batch 里的数据也是混合的
        return DataLoader(subset, batch_size=batch_size, shuffle=True)
    
class StagedCurriculumDataset(BaseDataset):
    def __init__(self, config, phase='train'):
        super().__init__(config, phase)
        # 缓存每个域的数据索引，避免重复计算，提升效率
    ...
    def get_loader(self, batch_size=None, shuffle=True, active_domain=None):
        """
        创建一个数据加载器 DataLoader。
        在训练阶段，可以根据 active_domain 参数筛选出特定域的数据。
        """
        if batch_size is None:
            if self.phase == 'train':
                batch_size = self.config['train']['batch_size']
        ...
        # 对于非训练阶段，或未指定 active_domain（或指定为 'all'），返回包含所有数据的加载器
        if self.phase != 'train' or active_domain is None or active_domain == 'all':
            self.logger.info(f"创建包含所有数据的 DataLoader (阶段: '{self.phase}')。")
            return DataLoader(self, batch_size=batch_size, shuffle=shuffle)

        # --- 核心：按域进行课程采样 ---

        # 1. 首次调用时，计算并缓存每个域的数据索引
        if not self._domain_indices:
            self.logger.info("首次按域筛选，正在计算并缓存索引...")
            # 假设 data[5] 是 domain_id 张量。如果你的意图是 data[0] (user_id)，请在此处修改。
            all_domain_ids = self.data[0]
            unique_domains = torch.unique(all_domain_ids)

            for domain_id_tensor in unique_domains:
                domain_id = domain_id_tensor.item()
                self._domain_indices[domain_id] = torch.where(all_domain_ids == domain_id)[0]
            self.logger.info(f"已缓存的域索引: {list(self._domain_indices.keys())}")

        # 2. 检查请求的 active_domain 是否有效
        if active_domain not in self._domain_indices:
            self.logger.warning(
                f"域 '{active_domain}' 在数据中未找到。将使用所有数据进行训练。"
            )
            return DataLoader(self, batch_size=batch_size, shuffle=shuffle)

        # 3. 获取当前活动域的索引
        indices_for_domain = self._domain_indices[active_domain]

        # 4. 使用筛选后的索引创建 Subset
        subset = Subset(self, indices_for_domain.tolist())
        self.logger.info(f"已为域 '{active_domain}' 创建 DataLoader。样本数: {len(subset)}")

        # 5. 返回对应子集的 DataLoader，shuffle=True 很重要
        return DataLoader(subset, batch_size=batch_size, shuffle=True)
